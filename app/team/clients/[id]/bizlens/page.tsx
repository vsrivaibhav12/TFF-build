...
export default async function ClientBizLensPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight">BizLens · {(client as any).business_name}</h1>
        <p className="text-zinc-500 mt-1">{months.length > 0 ? `${months.length} month(s) on record.` : 'No data yet — enter the first month below.'}</p>
      </div>
